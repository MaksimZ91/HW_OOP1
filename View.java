...
import org.example.Characters.BaseCharacter;
...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (BaseCharacter human: Main.heroList) {                            
            if (human.getCoords()[0] == x && human.getCoords()[1] == y){                                  
                if (human.getCurrentHealh() == 0) {
    ...
    public static void view() {
        ...
        step++;
        Main.heroList.forEach((v) -> l[0] = Math.max(l[0], v.toString().length()));
